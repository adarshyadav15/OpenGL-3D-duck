...
GLfloat theta[3] = { 0.0, 0.0, 0.0 }; //angle of each axis
...
GLboolean rotateState = false;
GLboolean runningState = false;
GLboolean runningState1 = false;
...
void idle()
{
	...
	//make the duck move smoothly
	if (runningState == true) {

		if (runningState1 == false) {
			randomY = rand() / (double)(RAND_MAX / 3) - 1.5;
			randomZ = rand() / (double)(RAND_MAX / 3) - 1.5;
			//printf("randomY is : %f\n", randomY);
			//printf("randomZ is : %f\n", randomZ);
			difY = randomY - posY;
			difZ = randomZ - posZ;
			runningState1 = !runningState1;
		}
		if (runningState1 == true) {
			posY += difY * transVelocity;
			posZ += difZ * transVelocity;
			if (fabs(posY - randomY) <= 0.02 || fabs(posZ - randomZ) <= 0.02) {
				runningState1 = false;
				runningState = false;
			}
		}
...
//set uo the special key function
...
void init()
{
	...
	glutAddMenuEntry("rotating", 1);
	glutAddMenuEntry("running", 2);
